...
<div class="mb-8">
    <div class="flex items-center space-x-2 text-sm text-stone-400 mb-2">
        ...
        <span>/</span>
        <span class="text-white">Edit Booking #{{ $booking->id }}</span>
    </div>
    <div class="flex justify-between items-end">
        <div>
            <h2 class="text-3xl font-bold text-white">Edit Booking #{{ $booking->id }}</h2>
            <p class="text-stone-400">Ubah data pemesanan meja atau aula</p>
        </div>
        <div>
             <span class="px-3 py-1 rounded-full text-sm font-bold {{ $booking->status === 'confirmed' ? 'bg-green-900/50 text-green-300 border border-green-500/30' : ($booking->status === 'pending' ? 'bg-yellow-900/50 text-yellow-300 border border-yellow-500/30' : ($booking->status === 'completed' ? 'bg-blue-900/50 text-blue-300 border border-blue-500/30' : 'bg-red-900/50 text-red-300 border border-red-500/30')) }}">
    ...
    <form action="{{ route('bookings.update', $booking->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <div class="flex gap-4">
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="type" value="table" class="peer hidden" {{ old('type', $booking->type ?? 'table') === 'table' ? 'checked' : '' }}>
                    ...
                    <label class="flex-1 cursor-pointer">
                        <input type="radio" name="type" value="aula" class="peer hidden" {{ old('type', $booking->type ?? 'table') === 'aula' ? 'checked' : '' }}>
            ...
            <div class="md:col-span-2">
                ...
                <input type="tel" name="phone" id="phone" value="{{ old('phone', $booking->phone) }}" required
                       class="w-full px-4 py-3 bg-stone-900 border border-stone-700 text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all placeholder-stone-600">
                @error('phone')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="date" id="date" value="{{ old('date', $booking->date) }}" required
                       min="{{ date('Y-m-d') }}"
            ...
            <div>
                ...
                <input type="time" name="time" id="time" value="{{ old('time', $booking->time) }}" required
                       class="w-full px-4 py-3 bg-stone-900 border border-stone-700 text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all color-scheme-dark">
                @error('time')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="people" id="people" value="{{ old('people', $booking->people) }}" required
                       min="1" max="50"
                       class="w-full px-4 py-3 bg-stone-900 border border-stone-700 text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all placeholder-stone-600">
                @error('people')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
            ...
            <div>
                <label for="status" class="block text-sm font-medium text-stone-300 mb-2">
                    <i class="fas fa-info-circle text-green-500"></i> Status <span class="text-red-500">*</span>
                </label>
            ...
            <div class="md:col-span-2">
                <label for="notes" class="block text-sm font-medium text-stone-300 mb-2">
                    <i class="fas fa-sticky-note text-green-500"></i> Catatan Khusus
                </label>
                <textarea name="notes" id="notes" rows="4"
                          class="w-full px-4 py-3 bg-stone-900 border border-stone-700 text-white rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all placeholder-stone-600">{{ old('notes', $booking->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
        ...
        <!-- Audit Info -->
        <div class="mt-8 p-4 bg-stone-700/30 border border-stone-700 rounded-xl text-sm text-stone-400">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div>
                    <i class="fas fa-calendar-plus mr-2 text-stone-500"></i> Dibuat pada: 
                    <span class="text-stone-300 font-mono">{{ date('d/m/Y H:i', strtotime($booking->created_at)) }}</span>
                </div>
                <div>
                    <i class="fas fa-calendar-check mr-2 text-stone-500"></i> Diubah terakhir: 
                    <span class="text-stone-300 font-mono">{{ date('d/m/Y H:i', strtotime($booking->updated_at)) }}</span>
                </div>
            </div>
        </div>

        <!-- Buttons -->
        <div class="flex justify-between items-center mt-8 pt-6 border-t border-stone-700">
             <button type="button" onclick="if(confirm('Yakin ingin menghapus booking ini?')) document.getElementById('delete-form').submit();"
                    class="px-6 py-3 border border-red-500/30 text-red-400 rounded-xl hover:bg-red-500/10 transition font-medium">
            ...
            <div class="flex space-x-4">
                ...
                <button type="submit" 
                        class="px-8 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-500 transition shadow-lg shadow-blue-900/20 font-bold tracking-wide">
